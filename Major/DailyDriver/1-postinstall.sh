...
echo
sudo sed "s|#governor=.*|governor='performance'|g" /etc/default/cpupower
sleep 1s
echo
echo "Setting minimum and maximum CPU Frequencies"
echo
sudo sed "s|#min_freq=.*|min_freq=\"3.7GHz\"|g" /etc/default/cpupower
sudo sed "s|#max_freq=.*|max_freq=\"4.2GHz\"|g" /etc/default/cpupower
sleep 1s
echo
...
echo
sudo sed "s|MAKEFLAGS=.*|MAKEFLAGS=\"-j$(expr $(nproc) \+ 1)\"|g" /etc/makepkg.conf
sleep 1s
sudo sed "s|COMPRESSXZ=.*|COMPRESSXZ=(xz -c -T $(expr $(nproc) \+ 1) -z -)|g" /etc/makepkg.conf
...
if pacman -Q | grep 'yakuake'; then
    ...
    if grep -q -F "DefaultProfile=" "/home/$(logname)/.config/yakuakerc"; then
        sed "s|DefaultProfile=.*|DefaultProfile=$(logname).profile|g" "/home/$(logname)/.config/yakuakerc"
